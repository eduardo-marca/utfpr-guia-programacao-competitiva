# Bem vindo ao Guia de Programação Competitiva UTFPR!

Esse guia tem como objetivo fornecer uma referência para os alunos de programação competitiva do UTFPR sobre os conceitos e principais algoritmos de programação competitiva.

Ele também serve como ponto de partida para quem está interessado em estudar programação competitiva, com guias voltados para iniciantes, como conceitos básicos de programação competitiva, C++ básico, algoritmos e técnicas básicas, entre outros.

!!! note "Site em Desenvolvimento"
    Este site ainda está em desenvolvimento e o que é apresentado é apenas um protótipo incompleto. Portanto praticamente tudo ainda pode ser modificado, incluindo páginas e sessões inteiras, estilos, estrutura do site, etc.

    Abaixo você encontra mais informações sobre como contribuir com este site.

## Como Usar Esse Guia
Esse guia é separado em diversas páginas. No topo você encontra os principais assuntos de programação competitiva separados por tópicos. Isso é útil se você estiver interessado em estudar um tópico específico, como C++, grafos ou algum algoritmo.

Você pode começar pela [Introdução à Programação Competitiva](introduction/what-is-cp.md), que explica o que é programação competitiva e como ela funciona.

Caso queira estudar todos os tópicos em uma ordem mais valtada para aprendizado, existe também a página [Trilha de Aprendizagem](introduction/learning-path.md) com os assuntos separados por níveis de dificuldade, em uma ordem recomendada para aprender.

## Como Contribuir
Caso esteja interessado, é possível contribuir com este guia. Para isso siga as instruções em [Como Contrubir
](contribute/index.md).
